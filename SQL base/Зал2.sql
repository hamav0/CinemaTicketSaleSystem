DECLARE @HallTable NVARCHAR(10) = 'Зал2';
...
BEGIN
    ...
    ELSE 15 END;
    SET @StandardGap = 12;
    DECLARE @CurrentPosX INT = 100 + (CASE WHEN @r > 6 THEN 0 ELSE 4 END) * (@SeatSize + @StandardGap); -- Базовый отступ ряда
    ...
    BEGIN
        ...
        IF @s = 3 AND @r > 6 AND @r < 9 SET @CurrentPosX = @CurrentPosX + @AisleWidth;
        ...
        IF @r in (7, 8) AND @s BETWEEN 9 AND 10 SET @Type = 2;
        ELSE IF @s BETWEEN 7 AND 8 and NOT @r in (7, 8) SET @Type = 2;
        ...
        IF @r = 9 SET @StandardGap = @StandardGap * 2;
        DECLARE @NextGap INT;
        IF @Type = 2 AND (@X = 0) 
        BEGIN
